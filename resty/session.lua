...
function session.new(opts)
    ...
    local o, i = pcall(include, "resty.session.storage." .. (y.storage or z.storage))
    if not o then
        i = include "resty.session.storage.cookie"
    end
    local o, j = pcall(include, "resty.session.serializers." .. (y.serializer or z.serializer))
    if not o then
        j = include "resty.session.serializers.json"
    end
    local o, k = pcall(include, "resty.session.encoders." .. (y.encoder or z.encoder))
    if not o then
        k = include "resty.session.encoders.base64"
    end
    local o, l = pcall(include, "resty.session.ciphers." .. (e or f))
    if not o then
        l = include "resty.session.ciphers.aes"
    end
    local self = {
